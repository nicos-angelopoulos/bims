...
% :- module( bims, [ bims/1, bims_lib/1 ] ).

:- use_module( library(requires) ).

% :- ensure_loaded( '../src/mcmcms' ).
:- requires( bims:mcmcms/12 ).
:- requires( bims:os_unique_by_date/2 ).
:- requires( bims:write_terms/2 ).
:- requires( bims:to_list/2 ).
:- requires( bims:n_digits/3 ).
:- requires( bims:clean_module/1 ).
:- requires( bims:report_triggers/2 ).
:- requires( bims:bims_bb_remove/2 ).
:- requires( bims:remove_template_duplicates/2 ).
:- requires( bims:get_date_time/1 ).
...
Opts 
	* chains(R=3)         number of chains or runs. Each chain is identified by N in 1...R.
	* iterations(I=100) number of iterations per run. Strictly speaking this is iterations - 1, that is, I is the number of models in each chain produced.
	* models(Models=carts) type of the models in the chain. An alternative type of model type is =|bns|=.
	* debug(Dbg=true)   If Dbg==true, run debug(bims) to get debuging messages. If Dbg==false, nodebug(bims) is called.
	* seeds(Seeds=1)    hash seeds for each run (1-1000), if length of Seeds is less than R, additional items added consequtively from last value.  So for instance, seeds(1) when chains(3) is given expands to seeds([1,2,3]).
	* likelihood(Lk=Model)   likelihood to use, default depends on Model chosen (system provided models, have a nameshake default likelihood, for example _carts_ likelihood is the default likelihood for carts models)
	* data(Data=Model)   a term that indicates the data for the runs. The precise way of loading and calls depend on Lk (the likelihood function) via the hook model_data_load/2, and what the prior (see option top_goal(Top)) expects. In general the dependency is with the likelihood, with the prior expected to be compatible with what the likelihood dictates in terms of data.  In the likelihoods provided, Data is the stem of a filename that is loaded in memory.  The file is looked for in Dir/Data[.pl] where Dir is looked for in [./data,bims(Model/data/)].
	* top_goal(Top=Model) the top goal for running the MCMC simulations. Should be the partial call corresponding to a predicate defined in Prior, as completed by adding the model as the last argument.
	* prior(Prior=Model) a file defining the prior DLP. Each model space has a default nameshake prior. The prior file is looked for in _dlps_ and bims(dlps).
	* backtrack(Backtract=uc) backtracking strategy (fix me: add details)
	* tempered(Tempered=[])   hot chains (fixme: add details) - this is an advanced feature undocumented for now
	* results_dir(Rdir=res-Dstamp)  results directory. If absent default is used. If present but a variable the default is used and returned as the instantiation to this variable.  The directory should not exist prior to the call. The default method uses a time stamp to provide uniqueness. (fixme: add prefix(Pfx) recognition)
	* report(These)     where These is a listable set of reportable tokens (should match 1st argument of bims:known_reportable_term/2). *[all|_]* or *all* is expanded to reporting all known reportable terms.
	* progress_percentage(Pc=10) the percentage at which to report progress of all runs (>100 or non numbers for no progress reporting)
	* progress_stub(Stub='.')    the stub marking progress
...
% bims_version( 1:1:0, date(2015,1,9) ).  % ? not 100% sure
bims_version(   1:1:1, date(2016,11,4) ). % development
...
bims_citation( Atom, bibtex(Type,Key,Pairs) ) :-
	Atom = 'Distributional Logic Programming for Bayesian Knowledge Representation. Nicos Angelopoulos and James Cussens. International Journal of Approximate Reasoning (IJAR).
To appear in Volume 80, January 2017, pages 52-66.',
    ...
    Pairs = [
			...
			journal= 'International Journal of Approximate Reasoning',
               year   = 2017,
               month  = 'January',
               url    = 'http://dx.doi.org/10.1016/j.ijar.2016.08.004'
     ].
% fixme add more
